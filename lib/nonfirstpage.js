...
const getNonfirstPage = module.exports = (nextpageLink, options, callback, redirect) => {
  const request = HTTPS.get(redirect ? redirect : `https://www.youtube.com${nextpageLink}&hl=en&disable_polymer=true`, { headers: options.headers }, resp => { // eslint-disable-line consistent-return, max-len
    if (resp.statusCode !== 200) {
      if (resp.statusCode === 302) return module.exports(playlistID, options, callback, resp.headers.location);
